...
* **2026-07-09** - **Dashboard** - Optimasi performa Dashboard dengan memisahkan API gambar (`/api/meals/[id]/image`) dan mengecualikan `image_url` Base64 raksasa dari respons `/api/dashboard`, sehingga load time jauh lebih cepat. Menggunakan `removeQueries` di scan agar UI menunjukkan status loading bukan data stale. - (Branch: `master`, Commit Msg: `perf: optimize dashboard API payload by lazy loading images via dedicated endpoint`)
* **2026-07-09** - **Report & Profile** - Optimasi respons API `/api/report` dengan mengecualikan payload Base64 `image_url` karena tidak dipakai di UI report, mempercepat load time secara drastis untuk riwayat data 30 hari. Menambahkan *cache-control bypass* pada *fetcher* Report dan Profile. - (Branch: `master`, Commit Msg: `perf: optimize report API payload and fix caching for report and profile`)
* **2026-07-13** - **Profile & Scan** - Fix bug Dashboard tidak terupdate secara otomatis setelah update profile pertama kali dan setelah scan AI. Menambahkan `queryClient.invalidateQueries(['dashboardData'])` dan `['profile']` di level *service mutation hook* (`useUpdateProfile` dan `useScanFood`) agar cache otomatis di-*refresh*. Menghapus `removeQueries` yang ada di komponen. - (Branch: `master`, Commit Msg: `fix: invalidate react-query cache on profile and scan mutations`)
